...
class CandidateMeta:
    id: str
    kind: str  # "identifier" | "global" | "specialized" | "detector"
    name: str
...
def _load_scene_spectrogram_cache(processed_dir: Path, scene: str) -> tuple[np.ndarray, np.ndarray, pd.DataFrame]:
    ...
    """
    from experiments.m3n_vc.training.trainer import normalize_spectrograms
    ...
    if not meta_path.exists():
        raise FileNotFoundError(
            ...
            f"`python -m experiments.m3n_vc.process_data --scene {scene}` first."
        )
...
def collect_empirical_outcomes(
    ...
    scene: "h24", "h08", "s31", "a06", "i29", or "i22". Requires
        ...
        to already exist (run `python -m experiments.m3n_vc.process_data --scene <scene>`
    ...
    payload = {
        ...
        "profile": HierarchyProfile(
            dataset_id=f"m3n_vc/{scene}",
            global_classes=tuple(GLOBAL_CLASS_NAMES),
            groups={
                "suv": ("gle350", "cx30"),
                "coupe": ("mustang", "miata"),
            },

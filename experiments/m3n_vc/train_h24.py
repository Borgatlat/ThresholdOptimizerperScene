...
from experiments.m3n_vc.models.spectrogram_cnn import (
    SpectrogramCNN,
    count_parameters,
)
from experiments.m3n_vc.process_data import save_h24_spectrogram_arrays
...
def load_processed(modality: str, processed_dir: Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    if not x_path.exists() or not y_path.exists():
        raise FileNotFoundError(
            f"Missing {x_path} or {y_path}. Run: "
            "python -m experiments.m3n_vc.train_h24 --preprocess"
        )
...
def main() -> None:
    ...
    if not args.preprocess and not args.train:
        ...
        print(
            "Example: python -m experiments.m3n_vc.train_h24 "
            "--preprocess --train --modality mic"
        )
        return
